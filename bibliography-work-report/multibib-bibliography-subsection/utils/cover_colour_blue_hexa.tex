\global\definecolor{BlueUniversity}{HTML}{436491}
...
\begin{tikzpicture}[remember picture,overlay]
	%%%%%%%%%%%%%%%%%%%% Background %%%%%%%%%%%%%%%%%%%%%%%%
	\fill[BlueUniversity] (current page.south west) rectangle (current page.north east);

	%%%%%%%%%%%%%%%%%%%% Background Polygon %%%%%%%%%%%%%%%%%%%%

	%\foreach \i in {2.5,...,10}
	%{
	%    \node[rounded corners,BlueUniversity!60,draw,regular polygon,regular polygon sides=6, minimum size=\i cm,ultra thick] at ($(current page.west)+(2.5,-5)$) {} ;
	%}

	\foreach \i in {0.5,...,22}
		{
			\node[rounded corners,BlueUniversity!90,draw,regular polygon,regular polygon sides=6, minimum size=\i cm,ultra thick] at ($(current page.north west)+(2.5,0)$) {} ;
		}

	\foreach \i in {0.5,...,14}
		{
			\node[rounded corners,BlueUniversity!90,draw,regular polygon,regular polygon sides=6, minimum size=\i cm,ultra thick] at ($(current page.north east)+(0,-9.5)$) {} ;
		}

	%\foreach \i in {21,...,6}
	%{
	%\node[BlueUniversity!85,rounded corners,draw,regular polygon,regular polygon sides=6, minimum size=\i cm,ultra thick] at ($(current page.south east)+(-0.2,-0.45)$) {} ;
	%}

\end{tikzpicture}

\vspace{-2cm}
\includegraphics[width=10cm]{university-white}\\[-1em]
\color{white}
...
\begin{tikzpicture}[remember picture,overlay]
	...
	%%%%%%%%%%%%%%%%%%%% Year %%%%%%%%%%%%%%%%%%%% 
	%\node[rounded corners,fill=BlueUniversity!70,text=white,regular polygon,regular polygon sides=6, minimum size=2.5 cm,inner sep=0,ultra thick] at ($(current page.west)+(2.5,-5)$) {\LARGE \bfseries 2020};

	\node[align=center,opacity=0.25] at ($(current page.south)+(6.5,1.38)$) {
		% LOGOS: https://university.es/identidad-corporativa/logos
